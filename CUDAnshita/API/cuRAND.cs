...
namespace CUDAnshita.API {
	...
	public class cuRAND : IDisposable {
		...
		*/
		...
		/// </returns>
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGenerate(IntPtr generator, IntPtr outputPtr, size_t num);
		...
		/// </returns>
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGenerateLogNormal(IntPtr generator, IntPtr outputPtr, size_t n, float mean, float stddev);

		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGenerateLogNormalDouble(IntPtr generator, IntPtr outputPtr, size_t n, double mean, double stddev);

		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGenerateLongLong(IntPtr generator, IntPtr outputPtr, size_t num);

		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGenerateNormal(IntPtr generator, IntPtr outputPtr, size_t n, float mean, float stddev);

		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGenerateNormalDouble(IntPtr generator, IntPtr outputPtr, size_t n, double mean, double stddev);

		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGeneratePoisson(IntPtr generator, IntPtr outputPtr, size_t n, double lambda);
		...
		/// </returns>
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandGenerateSeeds(IntPtr generator);
		...
		*/
		...
		/*
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandSetGeneratorOffset(IntPtr generator, unsigned long long offset);
		
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandSetGeneratorOrdering(IntPtr generator, curandOrdering_t order);
		*/
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandSetPseudoRandomGeneratorSeed(IntPtr generator, ulong seed);
		/*
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandSetQuasiRandomGeneratorDimensions(IntPtr generator, unsigned int num_dimensions);
		
		[DllImport(DLL_PATH, CallingConvention = CALLING_CONVENTION)]
		static extern curandStatus curandSetStream(IntPtr generator, cudaStream_t stream);
		*/
		...
		public int[] Generate(int num) {
			...
			CheckStatus(curandGenerate(generator, memory, num));
		...
		public float[] GenerateLogNormal(int num, float mean, float stddev) {
			...
			CheckStatus(curandGenerateLogNormal(generator, memory, num, mean, stddev));
		...
		public double[] GenerateLogNormalDouble(int num, double mean, double stddev) {
			...
			CheckStatus(curandGenerateLogNormalDouble(generator, memory, num, mean, stddev));
		...
		public long[] GenerateLong(int num) {
			...
			CheckStatus(curandGenerateLongLong(generator, memory, num));
		...
		public float[] GenerateNormal(int num, float mean, float stddev) {
			...
			CheckStatus(curandGenerateNormal(generator, memory, num, mean, stddev));
		...
		public double[] GenerateNormalDouble(int num, double mean, double stddev) {
			...
			CheckStatus(curandGenerateNormalDouble(generator, memory, num, mean, stddev));
		...
		public int[] GeneratePoisson(int num, double lambda) {
			...
			CheckStatus(curandGeneratePoisson(generator, memory, num, lambda));
		...
		public float[] GenerateUniform(int num) {
			...
			CheckStatus(curandGenerateUniform(generator, memory, num));
		...
		public double[] GenerateUniformDouble(int num) {
			...
			CheckStatus(curandGenerateUniformDouble(generator, memory, num));
		...
		IntPtr CreateGenerator(curandRngType type) {
			IntPtr generator = IntPtr.Zero;
			CheckStatus(curandCreateGenerator(ref generator, type));
			return generator;
		}

		IntPtr CreateGeneratorHost(curandRngType type) {
			IntPtr generator = IntPtr.Zero;
			CheckStatus(curandCreateGeneratorHost(ref generator, type));
			return generator;
		}

		void DestroyGenerator(IntPtr generator) {
			CheckStatus(curandDestroyGenerator(generator));
		}
		...
		void GenerateSeeds(IntPtr generator) {
			CheckStatus(curandGenerateSeeds(generator));
		}

		public static int GetVersion() {
			int version = 0;
			CheckStatus(curandGetVersion(ref version));
			return version;
		}
